...
const OPS: {
  ...
  },
  {
    ...
    label: "Scale X+Y",
    dual: true,
    enc: "N(D)=N(o)×(X+Y) mod T",
    dec: "N(o)=N(D)×(X+Y)⁻¹ mod T",
  },
  {
    id: "affine",
    label: "Affine ×X+Y",
...
function compLine(
  ...
  if (op === "affine" && dir === "decrypt") {
    ...
    return inv === null
      ? `(${step.idx}−${Y})×∄`
      : `(${step.idx}−${Y})×${inv}=${step.premod < 0 ? "?" : step.premod} mod ${T}=${step.ni}`;
  }
...
export default function CipherScreen() {
  ...
  const rawAlphabet = useMemo(
    () => buildAlphabet(groups, extraRaw, activePre, baseMode),
    [groups, extraRaw, activePre, baseMode],
  );
  const customBaseSize = useMemo(() => {
    const parsed = parseInt(customBase, 10);
    if (!Number.isFinite(parsed)) return 29;
    return Math.min(95, Math.max(2, parsed));
  }, [customBase]);
  const alphabet = useMemo(() => {
    if (baseMode !== "custom") return rawAlphabet;
    const next = [...rawAlphabet];
    const seen = new Set(next);
    for (let code = 32; code <= 126 && next.length < customBaseSize; code++) {
    ...
    return next.slice(0, customBaseSize);
  }, [baseMode, rawAlphabet, customBaseSize]);
  ...
  const run = useCallback(
    (dir: "encrypt" | "decrypt") => {
      try {
        if (!inputText.trim()) {
          setRunError("Enter input text before running cipher operations.");
          setResult(null);
          return;
        }
        if (alphabet.length < 2) {
          setRunError("Alphabet must contain at least 2 unique characters.");
          setResult(null);
          return;
        }
        const res = runAlphabetCipher(inputText, dir, {
          alphabet,
          op,
          keyX: X,
          keyY: Y,
        ...
        if (res.logs.some((l) => l.startsWith("error:"))) {
          setRunError(
            "Cipher engine returned an invalid configuration. Check alphabet and base settings.",
          );
          setResult(null);
          return;
        }
        ...
        setRunError((e as Error).message || "Unexpected cipher error");
        setResult(null);
      }
    },
  ...
  const togglePre = (ch: string) =>
    setActivePre((p) => {
      const n = new Set(p);
      n.has(ch) ? n.delete(ch) : n.add(ch);
      return n;
    });
  const toggleGroup = (k: keyof CharGroup) =>
    setGroups((p) => ({ ...p, [k]: !p[k] }));
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      >
        ...
        <View style={s.card}>
            ...
            {(
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (baseMode === "custom") toggleGroup(k);
                  }}
                >
                  <Text style={[s.chipTxt, on && s.chipTxtActive]}>
                    {on ? "✓ " : ""}
                    {l}
                  </Text>
                </TouchableOpacity>
          ...
          <View style={s.row}>
            {(["sensitive", "lower", "upper"] as CaseMode[]).map((cm) => (
              <TouchableOpacity
                key={cm}
                style={[s.chip, caseMode === cm && s.chipActive]}
                onPress={() => setCaseMode(cm)}
              >
                <Text style={[s.chipTxt, caseMode === cm && s.chipTxtActive]}>
                  {cm === "sensitive"
                    ? "Sensitive"
                    : cm === "lower"
                      ? "Lowercase"
        ...
        <View style={s.card}>
          ...
          <View style={s.row}>
            {EXTRA_PRE.map((ch) => (
              <TouchableOpacity
                key={ch}
                style={[s.chip, activePre.has(ch) && s.chipActive]}
                onPress={() => togglePre(ch)}
              >
                <Text style={[s.chipTxt, activePre.has(ch) && s.chipTxtActive]}>
                  {ch}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TextInput
            ...
            value={extraRaw}
            onChangeText={setExtraRaw}
            placeholder=", . ? ! extra symbols"
            placeholderTextColor={C.textMuted}
          />
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.row}>
            {(
              ...
              <TouchableOpacity
                key={b}
                style={[s.chip, baseMode === b && s.chipLime]}
                onPress={() => setBaseMode(b)}
              >
                <Text style={[s.chipTxt, baseMode === b && { color: C.lime }]}>
                  {l}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          {baseMode === "custom" && (
            <View style={[s.row, { marginTop: 7 }]}>
              ...
              <TextInput
                ...
                onBlur={() => setCustomBase(String(customBaseSize))}
              />
            </View>
          )}
          <View style={[s.row, { marginTop: 8 }]}>
            <View style={s.baseBadge}>
              <Text style={s.baseBadgeTxt}>BASE {T}</Text>
            </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.opGrid}>
            {OPS.filter((o) => !o.dual).map((o) => (
              <TouchableOpacity
                key={o.id}
                style={[s.opBtn, op === o.id && s.opBtnActive]}
                onPress={() => setOp(o.id)}
              >
                <Text style={[s.opLabel, op === o.id && s.opLabelActive]}>
                  {o.label}
                </Text>
        ...
        <View style={s.card}>
          <Text style={s.label}>
            KEY X <Text style={s.sub}>(0–{T - 1})</Text>
          </Text>
          <View style={s.keyRow}>
            <View style={s.keyDisplay}>
              <Text style={s.keyNum}>{X}</Text>
            </View>
            ...
            <TextInput
              style={[s.input, { width: 54, textAlign: "center" }]}
              keyboardType="number-pad"
              value={String(keyX)}
              onChangeText={(v) =>
                setKeyX(Math.max(0, Math.min(T - 1, parseInt(v) || 0)))
              }
            />
          </View>
          {isDual && (
            <>
              <Text style={[s.label, { marginTop: 10 }]}>
                KEY Y <Text style={s.sub}>(0–{T - 1})</Text>
              </Text>
              <View style={s.keyRow}>
                <View style={s.keyDisplay}>
                  <Text style={s.keyNum}>{Y}</Text>
                </View>
                <View style={s.keyBtns}>
                  {[-5, -1, 1, 5].map((d) => (
                    <TouchableOpacity
                ...
                <TextInput
                  ...
                  value={String(keyY)}
                  onChangeText={(v) =>
                    setKeyY(Math.max(0, Math.min(T - 1, parseInt(v) || 0)))
                  }
                />
              </View>
            </>
        ...
        {runError ? (
          <View style={[s.card, { borderColor: "rgba(244,63,94,0.45)" }]}>
          ...
          <View style={s.resultCard}>
            ...
            <View style={s.row}>
              <View
                style={[
                  s.badge,
                  result.dir === "encrypt" ? s.badgePurple : s.badgeCyan,
                ]}
              >
                <Text style={s.badgeTxt}>
                  {result.dir === "encrypt" ? "🔒 ENCRYPTED" : "🔓 DECRYPTED"}
                </Text>
              </View>
              <View style={s.badgeLime}>
                <Text style={s.badgeLimeTxt}>
                  BASE {T} · {opInfo.label}
                </Text>
              </View>
            </View>
            ...
            >
              ...
              >
                ...
                {isDual ? (
                  <Text style={{ color: C.lime }}> · Y={Y}</Text>
                ) : null}
            ...
            <View style={[s.row, { marginTop: 8, gap: 8 }]}>
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    await Share.share({ message: result.ciphertext });
                  } catch {
                    Alert.alert(
                      "Share failed",
              ...
              <TouchableOpacity
                style={s.ghostBtn}
                onPress={() => {
                  setInputText(result.ciphertext);
                  setResult(null);
                }}
              >
            ...
            {showSteps && (
              <View style={{ marginTop: 10 }}>
                <View style={s.tableHeader}>
                  ...
                  <Text style={[s.thCell, { flex: 0.8, textAlign: "right" }]}>
                    OUT
                  </Text>
                </View>
                {result.steps.map((step: any, i: number) => (
                  ...
                  >
                    <Text style={[s.tdChar, { flex: 0.8 }]}>
                      {step.inLabel}
                    </Text>
                    <Text style={[s.tdIdx, { flex: 0.8 }]}>
                      {step.pass ? "—" : step.idx}
                    </Text>
                    <Text style={[s.tdComp, { flex: 3 }]} numberOfLines={2}>
                      {compLine(step, op, result.dir, T, X, Y)}
                    </Text>
                    <Text style={[s.tdOut, { flex: 0.8 }]}>
                      {step.outLabel}
                    </Text>
                  </View>
                ))}
...
const s = StyleSheet.create({
  ...
  card: {
    margin: 10,
    marginBottom: 0,
    backgroundColor: "rgba(19, 17, 42, 0.74)",
    borderRadius: 12,
    padding: 14,
    borderWidth: 1,
    borderColor: C.border,
