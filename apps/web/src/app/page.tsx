...
const OPS: {
...
}[] = [
  {
    ...
    label: "Addition",
    dual: false,
    encFmt: "N(D) = N(o) + X mod T",
    decFmt: "N(o) = N(D) − X mod T",
  },
  {
    id: "sub",
    label: "Subtraction",
  ...
  },
  {
    ...
    label: "Scale (X+Y)",
    dual: true,
    encFmt: "N(D) = N(o) × (X+Y) mod T",
    decFmt: "N(o) = N(D) × (X+Y)⁻¹ mod T",
  },
  {
    id: "affine",
    label: "Affine (×X+Y)",
...
function buildAlphabet(
  groups: CharGroup,
  extraRaw: string,
  activePre: Set<string>,
  baseMode: BaseMode,
): string[] {
  if (baseMode === "alpha27") return [" ", ...UPPER];
  if (baseMode === "alpha26") return [...UPPER];
  if (baseMode === "ascii")
    return Array.from({ length: 128 }, (_, i) => String.fromCharCode(i));
  if (baseMode === "unicode")
    return Array.from({ length: 256 }, (_, i) => String.fromCodePoint(i));
...
function getFormulaLine(
  ...
  switch (opId) {
    ...
    case "mul":
      return dir === "encrypt"
        ? `N(D) = N(o) × ${X} mod ${T}`
        : `N(o) = N(D) × X⁻¹=${inv(X)} mod ${T}`;
    case "div":
      return dir === "encrypt"
        ? `N(D) = N(o) × X⁻¹=${inv(X)} mod ${T}`
        : `N(o) = N(D) × ${X} mod ${T}`;
    case "scale_sum": {
      const c = (((X + Y) % T) + T) % T;
      return dir === "encrypt"
        ? `N(D) = N(o) × (${X}+${Y}) mod ${T}  [= N(o) × ${c} mod ${T}]`
        : `N(o) = N(D) × (${X}+${Y})⁻¹=${inv(c)} mod ${T}`;
    }
    case "affine":
      return dir === "encrypt"
        ? `N(D) = N(o) × ${X} + ${Y} mod ${T}`
        : `N(o) = (N(D) − ${Y}) × X⁻¹=${inv(X)} mod ${T}`;
    case "scale_product": {
      const c = (((X * Y) % T) + T) % T;
      return dir === "encrypt"
        ? `N(D) = N(o) × (${X}·${Y}) mod ${T}  [= N(o) × ${c} mod ${T}]`
...
export default function CryptoCalcPage() {
  ...
  const effectiveT =
    baseMode === "custom" ? Math.max(2, customBase) : alphabet.length;
  ...
  const gcdWarn = useMemo(() => {
    ...
    let composite = X;
    if (op === "scale_sum")
      composite = (((X + Y) % effectiveT) + effectiveT) % effectiveT;
    if (op === "scale_product")
      composite = (((X * Y) % effectiveT) + effectiveT) % effectiveT;
    const g = gcd(composite, effectiveT);
    if (g !== 1) {
      const label =
        op === "scale_sum" ? "X+Y" : op === "scale_product" ? "X·Y" : "X";
      ...
      >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div className="panel" style={{ padding: "11px 13px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                {(
                  ...
                  ] as { key: keyof CharGroup; label: string; count: string }[]
                ).map((g) => {
                  const isOn =
                    baseMode === "alpha27"
              ...
              {!isCustom && (
                ...
                >
                  Switch to CUSTOM base to toggle groups
                </div>
              )}
            </div>
            ...
            <div className="panel" style={{ padding: "11px 13px" }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[
                  {
                    id: "sensitive" as CaseMode,
                    label: "Case Sensitive",
                    sub: "A ≠ a",
                  },
                  {
                    id: "lower" as CaseMode,
                    label: "Lowercase",
                    sub: "A → a = 26",
                  },
                  {
                    id: "upper" as CaseMode,
                    label: "UPPERCASE",
                    sub: "a → A = 26",
                  },
                ].map((cm) => (
            ...
            <div className="panel" style={{ padding: "11px 13px" }}>
              ...
              >
                Appended after group chars · duplicates ignored
              </div>
            </div>
            ...
            <div className="panel" style={{ padding: "11px 13px" }}>
              ...
              {isCustom && (
                ...
                >
                  ...
                  <input
                    ...
                    value={customBase}
                    onChange={(e) =>
                      setCustomBase(Math.max(2, parseInt(e.target.value) || 2))
                    }
                  />
                </div>
              )}
              ...
              >
                ...
                {gcdWarn && (
                  <div
                    className="gcd-warning"
                    style={{ flex: 1, padding: "4px 10px", fontSize: 10 }}
                  >
                    ⚠ {gcdWarn}
                  </div>
                )}
              ...
              >
                {OPS.filter((o) => o.dual).map((o) => (
                  <button
                    ...
                    style={
                      o.id === "affine" &&
                      OPS.filter((x) => x.dual).length % 2 !== 0
                        ? { gridColumn: "span 1" }
                        : {}
                    }
                  >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setKeyX(
                      Math.max(
                        0,
                        Math.min(effectiveT - 1, parseInt(e.target.value) || 0),
                      ),
                    )
                  }
                />
        ...
        {result ? (
          <div className="panel animate-in" style={{ padding: "13px 15px" }}>
            ...
            >
              <span
                className={`result-badge ${lastDir === "encrypt" ? "badge-encrypted" : "badge-decrypted"}`}
              >
              ...
              <button
                id="copy-result-btn"
                className="btn-ghost"
                onClick={() => navigator.clipboard.writeText(result.ciphertext)}
              >
                📋 Copy result
              </button>
              <button
                ...
                className="btn-ghost"
                style={{
                  borderColor: showFormula ? "var(--accent-purple)" : undefined,
                }}
            ...
            >
              ...
              {isDual && (
                <>
                  ...
                  <span style={{ color: "var(--accent-lime)" }}>Y = {Y}</span>
                </>
              )}
              <span style={{ color: "var(--text-muted)" }}> · </span>
                  ...
                  <tbody>
                    {result.steps.map((step: ComputeStep, i: number) => (
                      <tr key={i}>
                        ...
                        <td className={step.pass ? "td-pass" : "td-idx"}>
                          {step.pass ? "—" : step.idx}
                        </td>
                        ...
                        <td className={step.pass ? "td-pass" : "td-out"}>
                          {step.outLabel}
                        </td>
                      </tr>
                    ))}
          ...
          >
            ...
            <div style={{ fontSize: 11 }}>
              Configure your settings above, then hit Encrypt or Decrypt
            </div>
          </div>
        )}
      </div>
